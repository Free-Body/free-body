...
export const startFreeFloat = function () {
  ...
  addObjectButton.addEventListener("click", function () {
    ...
    let randomX = Math.random() * render.options.width;
    let randomY = Math.random() * render.options.height;
    Body.setPosition(circle, { x: randomX, y: randomY });
  ...
  Events.on(engine, "afterUpdate", function () {
    for (let i = 0; i < buoyantObjects.length; i++) {
      ...
      let speed = Math.sqrt(
        Math.pow(obj.velocity.x, 2) + Math.pow(obj.velocity.y, 2)
      );
      if (speed < 0.5) {
        // Apply "braking force"
      ...
      if (obj.density < waterDensity) {
        ...
        var adjustedBuoyancyForce =
          submergedVolume * (waterDensity - obj.density) * 0.0001;
        ...
        Body.applyForce(obj, obj.position, {
          x: 0,
          y: 0.0001 - adjustedBuoyancyForce,
        });
      }
    }
  });
